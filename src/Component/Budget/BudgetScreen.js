...
import { addBudgetYear, addBudgetYearReset, getAllBudgetYears } from '../../features/budget/budgetSlice';
import BudgetYears from './BudgetYears';
import EditBudgetForm from './EditBudgetForm';

function BudgetScreen(props) {
    const user = useSelector((state) => state.user.userInfo);
    const years = useSelector((state) => state.budget.getAllBudgetYears);
    const addYearStatus = useSelector((state) => state.budget.addBudgetYear);
    ...
    useEffect(() => {
        if(storeId){
            ...
            dispatch(getAllBudgetYears(data));
        }
    }, [storeId,reload])
    ...
    const incrementReload = () => {
      ...
      const handleAddYear = () => {
        setAddYearShow(true);
        dispatch(addBudgetYear({storeId,token:user.access_token}));
      }
